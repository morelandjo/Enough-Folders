...
/**
 * Core integration class for JEI mod.
 * Acts as a facade for the various JEI integration components.
 */
public class JEIIntegrationCore implements ModIntegration, IngredientDragProvider, RecipeViewingIntegration {
        ...
        try {
            ...
            if (isAvailable()) {
                ...
                folderScreen.registerIngredientClickHandler((slot, button, shift, ctrl) -> 
                    recipeManager.handleIngredientClick(slot, button, shift, ctrl));
    ...
    public boolean isTransitioningToRecipeScreen(Screen screen) {
        return recipeManager.isTransitioningToRecipeScreen(screen);
    }
